  ...
  if ((ctx.request && ctx.request.header && ctx.request.header.authorization)  ||
    ...
    try {
      ...
      if (ctx.request.query && ctx.request.query.token) {
        ...
        const [token] = await strapi.query('user-tokens').find({token: ctx.request.query.token});
      ...
      if (isAdmin) {
        ...
        ctx.state.user = await strapi.query('user', 'users-permissions').findOne({ id }, ['role','organizations']);
      }
    } catch (err) {
